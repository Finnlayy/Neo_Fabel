#!/usr/bin/env python3
"""Generate Pine indicator variants from local GA result files."""
...
DEFAULT_TEMPLATE = WORKDIR / "ETH_USDT_GlintNews_PionexBot_v5_indicator.pine"
...
def replace_input_default(text: str, var_name: str, value: str, kind: str) -> str:
    pattern = rf"{var_name} = input\.{kind}\([^,]+,"
...
def extract_template_symbol(template_text: str, fallback: str) -> str:
    match = re.search(r'indicator\(title="([^"]+)"', template_text)
    if not match:
        return fallback
    title = match.group(1)
...
def apply_candidate(
    ...
    output = replace_once(output, r'title="[^"]+"', f'title="{title}"')
    output = replace_once(output, r'shorttitle="[^"]+"', f'shorttitle="{shorttitle}"')
    output = replace_once(
        output,
    ...
    if ignored_keys:
        ...
        output = output.replace(
            marker,
            marker + f"// Nicht direkt auf den Indicator gemappt: {', '.join(ignored_keys)}\n",
            1,
        )

    output = replace_optional(output, r'table\.cell\(dash, 0, 0, "[^"]+"', f'table.cell(dash, 0, 0, "{shorttitle}"')
    output = replace_optional(output, r"// Datei: .+\n", f"// Datei: Top{rank}_{symbol_file_tag}_GA_Optimized.pine\n")
    output = replace_optional(output, r"// Zweck: .+\n", f"// Zweck: GA-optimierter Pionex Signal Bot Indicator fuer {symbol_label}.\n")
    return output
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate GA-tuned Pine indicators from local optimizer results.")
    parser.add_argument("--results", type=Path, default=DEFAULT_RESULTS, help="Path to a GA results JSON file")
    parser.add_argument("--template", type=Path, default=DEFAULT_TEMPLATE, help="Pine template used as the base indicator")
