"""Generate Pine v6 variants from GA result payloads (ported from generate_ga_pines.py)."""
...
DEFAULT_TARGET_SYMBOL = "ETH/USDT"
DEFAULT_TF_HIGH = "48"
DEFAULT_TF_MID = "12"
DEFAULT_TF_LOW = "3"
...
# Usable GA bake templates (id → relative filename under assets/strategies).
TEMPLATE_REGISTRY: dict[str, dict[str, str]] = {
    "eth_glintnews_pionex_v6": {
        "id": "eth_glintnews_pionex_v6",
        "filename": "ETH_USDT_GlintNews_PionexBot_v6_strategy.pine",
        "label": "ETH USDT GlintNews PionexBot v6",
    },
}
...
def resolve_template_path(template_id: str) -> Path:
    ...
    path = STRATEGIES_DIR / meta["filename"]
    if not path.exists():
...
def enforce_trading_invariants(text: str) -> str:
    """Align strategy() header with GA comparability invariants (no logic changes)."""
    if "strategy(" not in text:
        return text
    out = text
    out = re.sub(r"initial_capital\s*=\s*[0-9.]+", "initial_capital = 100", out, count=1)
    out = re.sub(
        r"default_qty_type\s*=\s*strategy\.\w+",
        "default_qty_type = strategy.percent_of_equity",
        out,
        count=1,
    )
    out = re.sub(r"pyramiding\s*=\s*\d+", "pyramiding = 1", out, count=1)
    return out


def export_pines_from_payload(
    ...
    *,
    ...
    limit: int = 3,
    fallback_symbol: str = DEFAULT_TARGET_SYMBOL,
    target_symbol: str = DEFAULT_TARGET_SYMBOL,
    tf_high: str = DEFAULT_TF_HIGH,
    tf_mid: str = DEFAULT_TF_MID,
    tf_low: str = DEFAULT_TF_LOW,
) -> list[dict[str, Any]]:
    """Bake genes into a bundled template; write under output_dir. Returns file metadata."""
    path = template_path or resolve_template_path(template_id)
    ...
    for rank, candidate in enumerate(candidates, start=1):
        ...
        body = apply_candidate(
            template_text, candidate, rank, selected_symbol, tf_high, tf_mid, tf_low
        )
        body = enforce_trading_invariants(body)
...
def generate_files(
    ...
    return [Path(row["path"]) for row in meta]
