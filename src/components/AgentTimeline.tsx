...
const TimelineItem = ({ ev }: { ev: TimelineEvent, key?: React.Key }) => {
...
const TimelineFeed = ({ filteredEvents }: { filteredEvents: TimelineEvent[] }) => (
...
const EventInjectionForm = ({
  onInjectEvent,
}: {
  onInjectEvent: (event: Omit<TimelineEvent, 'id' | 'time'>) => void;
}) => {
...
export default function AgentTimeline() {
  ...
  const handleInjectEvent = (eventData: Omit<TimelineEvent, 'id' | 'time'>) => {
  ...
  return (
    <div id="agent-timeline-card" className="bg-slate-900/40 border border-white/5 hover:border-white/10 rounded-xl p-5 font-mono text-xs space-y-4 transition-all duration-300">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-5">
        <TimelineFeed filteredEvents={filteredEvents} />
        <EventInjectionForm onInjectEvent={handleInjectEvent} />
      </div>
