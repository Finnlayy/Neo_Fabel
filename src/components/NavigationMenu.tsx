...
function getTabs(language: "en" | "de"): TabConfig[] {
  return [
    {
      id: "dashboard",
...
function useKeyboardNavigation(setActiveTab: (tab: MainTab) => void) {
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case "2": setActiveTab("terminal"); break;
        case "3": setActiveTab("strategy"); break;
        case "4": setActiveTab("swarm"); break;
        case "5": setActiveTab("signals"); break;
        case "6": setActiveTab("academy"); break;
        default: break;
      }
    };
...
function NavigationTabItem({ tab, isActive, index, language, onClick }: NavigationTabItemProps) {
  const Icon = tab.icon;
  return (
    <button
      role="tab"
    ...
    >
      {isActive && (
        <div className={`absolute top-0 left-0 right-0 h-[2px] ${ACCENT_BAR[tab.accent]}`} />
      )}

      <Icon className={`w-4.5 h-4.5 transition-transform duration-300 group-hover:scale-110 shrink-0 ${
        isActive ? ACCENT_TEXT[tab.accent] : "text-slate-500 group-hover:text-slate-400"
      }`} />
...
function MetricsDisplay({ activeSymbol, winLossRatio, executedCount, isComplianceActive, language }: MetricsDisplayProps) {
  return (
...
export default function NavigationMenu({
  ...
  useKeyboardNavigation(setActiveTab);
