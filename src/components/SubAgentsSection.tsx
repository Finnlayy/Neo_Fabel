...
interface SubAgentCardProps {
  ...
  setRiskMultiplier: (val: number) => void;
  onOptimizeThresholds: (val: number) => void;
  onToggleCompliance: () => void;
  handleAnalyzeTrades: () => Promise<void>;
  isAnalyzing: boolean;
  onUpdateAgentStatus: (id: string, status: SubAgentState["status"]) => void;
}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {agents.map((agent) => (
          <SubAgentCard
            ...
            onOptimizeThresholds={onOptimizeThresholds}
            onToggleCompliance={onToggleCompliance}
            handleAnalyzeTrades={handleAnalyzeTrades}
